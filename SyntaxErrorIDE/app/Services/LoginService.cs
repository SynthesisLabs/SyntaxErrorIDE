...
public class LoginService
{
    ...
    {
        var reader = Conn.GetReader("SELECT id, password FROM users WHERE name = @name",
    ...
    public string Register(string? name, string? email, string? password, string? passwordRepeat)
    {
        ...
        var emailReader =
            Conn.GetReader("SELECT * FROM users WHERE email = @email", new MySqlParameter("@email", email));
